...
- [x] **Step 3: 建立 Popover 与 Dialog 行为契约**
...
- [x] **Step 5: 验证键盘、指针和 reduced-motion**

手动使用 Tab、Shift+Tab、Enter、Space、Escape 检查新增原语；在 Chrome DevTools 切换触摸模拟与 reduced-motion，确认 hover 不会在触摸设备粘滞、焦点不会丢失。

验证记录：已完成组件焦点契约审查、reduced-motion/hover 媒体规则扫描，以及覆盖现有原语的 30 个桌面与移动端 Playwright 场景。新增原语尚未接入业务页面，实际页面中的 Tab/Escape/焦点返回需在首次迁移使用时再次复验；本轮遵循 Test Addition Policy，不为孤立组件增加测试入口。
