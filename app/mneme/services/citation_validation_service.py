    ...
    *,
    ...
    if summary["valid_citation_count"] > 0:
        if summary["invalid_citation_count"] > 0:
            note = "Some citations failed validation and were removed from citations."
            return confidence, append_uncertainty(uncertainty, note)
        return confidence, uncertainty

    if summary["source_count"] == 0:
        return "low", append_uncertainty(uncertainty, "No sources were retrieved.")

    if summary["draft_citation_count"] == 0:
        return "low", append_uncertainty(uncertainty, "The answer did not include draft citations.")

    return "low", append_uncertainty(uncertainty, "Citation source_id or quote did not match retrieved sources.")
