...
from app.mneme.domains.retrieval.context_items import (
...
def can_merge_documents(
    ...
    *,
    ...
    consecutive = isinstance(prev_index, int) and isinstance(current_index, int) and current_index == prev_index + 1
    ...
    page_close = not (
        isinstance(prev_page, int) and isinstance(current_page, int) and abs(current_page - prev_page) > 1
    )
    if not page_close:
        return False
...
async def build_query_context(
    ...
    *,
    ...
    vector_items = [build_context_item_from_vector(doc, score) for doc, score in merged_vector_docs]
    ...
    return {
        ...
        "sources": [build_source_item(doc, source_id=f"S{index}") for index, doc in enumerate(final_docs, start=1)],
