...
test("mobile drawers and upload are keyboard operable with focus restoration", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Files", exact: true })).toHaveAttribute("aria-controls", "document-tree-pane");
  await page.getByRole("button", { name: "Files", exact: true }).press("Escape");
  ...
  await expect(page.getByRole("button", { name: "Properties", exact: true })).toHaveAttribute("aria-controls", "document-properties-pane");
  await page.keyboard.press("Escape");
