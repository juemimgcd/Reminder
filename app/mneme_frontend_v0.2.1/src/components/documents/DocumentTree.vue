...
import { FileText, FolderInput, Pencil, Plus, Trash2, Upload } from "@lucide/vue";
...
const parentForNewFolder = computed(() => rootFolder.value?.id || "");
...
<template>
  <aside data-testid="document-tree-pane" class="tree-pane">
    <header>
      <div><small>{{ t("vault.active") }}</small><h2>{{ t("reader.files") }}</h2></div>
      <label class="upload-control" for="workspace-upload" title="Upload document"><Upload /></label>
    </header>
    ...
    <div v-if="movingFolder && selectedFolder" class="folder-move-menu" role="listbox" aria-label="Move folder">
      <button
        ...
        role="option"
        :aria-selected="false"
        @click="emit('moveFolder', selectedFolder.id, target.id); movingFolder = false"
    ...
    <nav data-testid="document-tree" class="tree-scroll" aria-label="Document files">
      <ul role="tree" aria-label="Vault folders">
        <li v-for="document in rootDocuments" :key="document.id" role="treeitem" class="root-document" :class="{ active: activeDocumentId === document.id }">
          ...
          <button type="button" :aria-label="`Move ${document.file_name}`" @click="movingRootDocumentId = movingRootDocumentId === document.id ? '' : document.id"><FolderInput /></button>
          <div v-if="movingRootDocumentId === document.id" class="root-move-menu" role="listbox" aria-label="Move document">
            <button
      ...
      <p v-if="!latestDocuments.length" class="tree-empty">No documents yet.</p>
    </nav>
  </aside>
</template>
...
.upload-control { display: grid; width: 2rem; height: 2rem; place-items: center; color: var(--text-secondary); border: 1px solid var(--border-muted); border-radius: 0.35rem; cursor: pointer; }
