...
def test_list_documents_joins_owned_same_kb_folder_in_one_query():
    database = StatementCaptureDatabase()

    asyncio.run(list_documents(database, user_id=7, knowledge_base_pk=11))
...
def test_document_list_serializes_public_folder_and_version_metadata(monkeypatch):
    ...
    monkeypatch.setattr(documents_router, "list_documents", listed)
