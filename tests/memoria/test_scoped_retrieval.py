...
def test_memory_retriever_maps_current_rows_to_scoped_evidence():
    with patch(
        "app.mneme.memoria.server.retrieval.memories.open_read_session",
...
def test_document_retriever_passes_scope_and_top_k_to_both_recall_ports():
    ...
    with (
        patch("app.mneme.memoria.server.retrieval.documents.search_vector", vector),
        patch("app.mneme.memoria.server.retrieval.documents.search_keyword", keyword),
        patch("app.mneme.memoria.server.retrieval.documents.open_read_session", return_value=_ReadSession()),
    ):
