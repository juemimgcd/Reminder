...
from clients.neo4j_client import is_neo4j_projection_enabled, run_neo4j_write
...
from services.graph_service import _build_related_document_edges
...
async def sync_user_projection(*, user) -> None:
...
async def sync_knowledge_base_projection(*, user, knowledge_base) -> None:
    await _execute_projection_write(
...
async def sync_document_projection(*, user, knowledge_base, document) -> None:
    await _execute_projection_write(
...
async def sync_document_memory_projection(
    ...
    if rebuild_related_edges:
        await rebuild_user_related_projection(db, user_id=user.id)
