...
```text
浏览器 -> Mneme FastAPI -> Memory Agent API -> DeepSeek
                 \-> PostgreSQL Outbox -> Memory Agent worker
                                      \-> BGE-M3 + pgvector
```

Mneme 负责用户、文档和任务编排，Memory Agent 负责检索、记忆治理与回答生成。已提交的
`memoria.json` 统一使用 DeepSeek；BGE-M3 负责语义向量，向量持久化在独立
`memory_agent` PostgreSQL 数据库的 pgvector 中。
...
- 多知识库管理
- 文档上传、解析、切分和索引
- Memory Agent、BGE-M3 和 pgvector 驱动的向量检索与 RAG 问答
- PostgreSQL Outbox/Inbox 保证文档投影、删除和记忆事件可重试、可追踪
- Neo4j 图谱投影、查询和重建
- 记忆库、个人画像、成长报告和建议生成
- Celery 异步任务队列
- Durable Agent Run、定时 Heartbeat、事件唤醒和站内通知
- Vue 3 前端工作台，由 FastAPI 统一托管
- Docker Compose 部署 PostgreSQL/pgvector、Redis、Neo4j、应用、beat、业务 worker 和 Memory Agent

## 技术栈

- 后端：FastAPI、Pydantic v2、SQLAlchemy Async、Alembic
- 前端：Vue 3、TypeScript、Vite、D3
- 存储与队列：PostgreSQL/pgvector、Redis、Neo4j、Celery；Milvus 为可选兼容 profile
- AI/RAG：DeepSeek、BGE-M3、LangChain、sentence-transformers、reranker
...
│   │   ├── memoria/              # Memory Agent 契约、客户端、Outbox 与独立服务
...
├── requirements/                 # 分组依赖
├── tests/                        # 回归测试
├── memoria.json                  # Agent 模型、检索、推理、工具与 Multi-Agent 配置
├── main.py                       # 应用入口
├── start.sh                      # 本地联动启动脚本
└── docker-compose.yml
```
...
- `.env`：后端与部署配置，例如数据库、JWT、Redis、Celery、Neo4j、Milvus 和 API 密钥。
- `memoria.json`：Agent 配置，例如模型选择、上下文预算、检索、推理、工具和 Multi-Agent 策略。

`memoria.json` 可以通过 `${VAR_NAME}` 引用 `.env` 中的密钥，默认路径为项目根目录，
也可通过后端变量 `MEMORIA_CONFIG_PATH` 指定其他位置。Agent 配置以
`memoria.json` 为准，旧的 Agent 环境变量不会覆盖它。当前提交配置中的聊天、
记忆提取和回答模型均为 DeepSeek，空白或无效 provider 也会回退到 DeepSeek；
运行时至少需要配置 `DEEPSEEK_API_KEY`。
...
Memory Agent 默认使用 `BAAI/bge-m3`，并在 API 启动时预加载模型。以下变量可以覆盖默认行为：

- `MEMORY_AGENT_EMBEDDING_MODEL_NAME`
- `MEMORY_AGENT_EMBEDDING_MODEL_PATH`
- `MEMORY_AGENT_EMBEDDING_CACHE_DIR`
- `MEMORY_AGENT_EMBEDDING_LOCAL_FILES_ONLY`
...
- Memory Agent 拥有启用 pgvector 的 `${MEMORY_AGENT_POSTGRES_DB:-memory_agent}`、Redis DB 2/3 和 `${MEMORY_AGENT_CELERY_QUEUE:-memory_agent}` 队列。
- Durable Agent Run 的短期协调状态独立使用 Redis DB 4。
...
首次启动会下载约 2.3 GB 的 BGE-M3 模型；缓存持久化在 `storage` volume 中，后续重建
容器不会重复下载。Memory Agent readiness 会等待模型预加载和依赖检查完成。

只有需要兼容旧 Mneme Milvus 向量后端时才启用 `vector` profile；在线 Memory Agent
问答默认使用 pgvector，不需要这个 profile：
...
发布脚本会拉取版本化镜像，初始化 Memory Agent 数据库，依次执行主库和 Memory Agent
迁移，重建 `app`、`worker`、`beat`、`memory-agent-api` 和
`memory-agent-worker`，最后检查应用健康、Memory Agent readiness 与 worker。
PostgreSQL 默认使用固定版本的 `pgvector/pgvector:0.8.5-pg17-bookworm`，避免
`latest` 漂移。

回滚时使用上一版 tag 重新执行同一个脚本；不要删除 Outbox/Inbox、投影、记忆或删除
fence 数据：
...
answer runs.
Mneme-local Outbox age/dead-letter diagnostics are available with:
