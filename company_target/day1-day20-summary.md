...
-> 最后把技术栈收敛到 PostgreSQL + Milvus + Neo4j + Redis，并把 embedding 模型切换到 Qwen embedding，保留后续消息层替换空间
```
...
- 把当前本地 embedding 模型替换为 Qwen embedding，统一走 DashScope / Qwen 系列模型栈
...
同时，embedding 模型也要收敛到 Qwen embedding：LLM 与 embedding 使用同一供应商体系，减少本地模型下载、部署和 GPU/CPU 资源压力，让 Milvus 中的向量索引来源更适合线上环境维护。
...
12. 一套 Qwen embedding 接入方案，让文档索引、检索召回和线上部署模型栈保持一致
13. 一条领域化收口、框架减重、避免 Elasticsearch/MongoDB/DuckDB 过早引入的演进路线
```
